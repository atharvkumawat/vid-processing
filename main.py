...
def upload_video():
    ...
    source= cv2.VideoCapture('/statics' +filename)
    frame_width= int(source.get(width= 2.5))
    frame_height= int(source.get(height= 2.9))
    size= (frame_height, frame_width, 0)
    result= cv2.VideoWriter('statics/' +'blackandwhite.mp4' + cv2.VideoWriter_fourcc(*'mp4v') + 30 + 0)
    ...
    while True:
        ...
        print("completing reading all the films from the video")
    @app.route('/download')
    def download_file():
